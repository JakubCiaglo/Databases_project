...
author:
  ...
  - "<span style='font-size: 0.9em; color: #7f8c8d;'>Jakub Ciągło</span>"
  - "<span style='font-size: 0.9em; color: #7f8c8d;'>Mateusz Ćwiek</span>"
  - "<span style='font-size: 0.9em; color: #7f8c8d;'>Marek Dworaczyk</span>"
  - "<span style='font-size: 0.9em; color: #7f8c8d;'>Rafał Głodek</span>"
lang: pl-PL
date: "2125-06-13"
date-format: "long"
format:
...
Analiza została przeprowadzona na podstawie pełnych danych operacyjnych i finansowych z pięciu lat działalności naszej firmy. Wykorzystaliśmy do tego bezpośrednie zapytanie do naszej centralnej bazy danych, agregując informacje o liczbie lotów, całkowitych kosztach misji oraz skompletowanych transakcjach.
...
if unprofitable_types.empty:
    rentownosc_markdown += (
        "<p><b style='color: #198754;'>Doskonałe wieści:</b> W analizowanym okresie wszystkie oferowane przez Space-U rodzaje wypraw "
        "okazały się rentowne. To świadczy o zdrowej strukturze kosztów i dobrze skalibrowanej polityce cenowej.</p>"
    )
else:
    rentownosc_markdown += (
        "<p><b style='color: #dc3545;'>Uwaga Zarządcza:</b> Zidentyfikowano następujące rodzaje wypraw, które generują straty. "
...
Analiza miesięcznej liczby obsłużonych klientów jest jednym z fundamentalnych wskaźników kondycji i tempa rozwoju Space-U. To część naszej działalności, która pozwala ocenić skuteczność naszych działań marketingowych, atrakcyjność oferty oraz ogólne zainteresowanie podróżami kosmicznymi. W tej sekcji zbadamy, czy w ostatnich latach nasza firma znajdowała się na trajektorii wzrostowej, czy też napotkała na turbulencje.

Aby odpowiedzieć na to pytanie, stworzyliśmy wizualizację przedstawiającą liczbę klientów obsużonych w każdym miesiącu. Zamiast polegać na subiektywnej ocenie, nałożyliśmy na dane linię trendu opartą na regresji liniowej, która w obiektywny sposób wskazuje kierunek, w którym zmierzamy.
...
<p style='{color}'><b>Wniosek: {wniosek}</b></p>
<p>Wyjaśnienie: {szczegoly}</p>
"""
...
display(df_risk.style.format('{:.2f}', subset=['Wazony_wspolczynnik_ryzyka'])\
                 ...
                 .set_caption("Ranking Ryzyka Destynacji (Wskaźnik ważony)"))
```
...
<p>Nasz ważony wskaźnik pozwala na obiektywną ocenę i priorytetyzację działań redukujących ryzyko.</p>
<ul>
    <li><span style="font-size: 1.5em;"></span> <b style='color: #dc3545;'>Najwyższe ryzyko:</b> Destynacją o najwyższym wskaźniku ryzyka jest <b>{most_risky['Kierunek']}</b> z wynikiem <b>{most_risky['Wazony_wspolczynnik_ryzyka']:.2f} pkt</b> na wyprawę. Sugeruje to konieczność przeglądu procedur bezpieczeństwa dla tej trasy.</li>
    <li><span style="font-size: 1.5em;"></span> <b style='color: #198754;'>Najniższe ryzyko:</b> Najbezpieczniejszą z naszych popularnych destynacji okazał/a się <b>{least_risky['Kierunek']}</b> ({least_risky['Wazony_wspolczynnik_ryzyka']:.2f} pkt/wyprawę). Praktyki stosowane na tej trasie mogą stanowić wzór dla innych.</li>
</ul>
"""
...
fig = px.bar(
    ...
    color='usterki_na_wyprawe',
    color_continuous_scale='Reds',
    height=600,
    text='usterki_na_wyprawe'
)
...
wniosek_markdown += f"""
<li><b style='color: #dc3545;'>Wymaga uwagi:</b> Najbardziej podatny na usterki sprzętowe jest statek <b>{most_failure_prone['nazwa_statku']}</b>. 
...
wniosek_markdown += f"""
<li><b style='color: #198754;'>Wzór niezawodności:</b> Aż <b>{count_zero_failure} jednostek</b> w naszej flocie zakończyło wszystkie misje w analizowanym okresie bez żadnej usterki sprzętowej.
...
fig_dest.update_layout(showlegend=False,xaxis=dict(range=[0, 5.2]), plot_bgcolor='white', yaxis={'categoryorder':'total ascending'})
fig_dest.show()
```
...
<ul>
    <li><b style='color: #198754;'>Najwyżej oceniana:</b> Destynacją, która dostarcza naszym klientom najwięcej satysfakcji, jest <b>{best_dest['kierunek']}</b> ze średnią oceną <b>{best_dest['srednia_ocena']:.2f}</b> (na podstawie {int(best_dest['liczba_ocen'])} opinii).</li>
    <li style="margin-top: 15px;"><b style='color: #dc3545;'>Wymaga uwagi:</b> Najniższe oceny otrzymuje <b>{worst_dest['kierunek']}</b> (średnia <b>{worst_dest['srednia_ocena']:.2f}</b> z {int(worst_dest['liczba_ocen'])} opinii). Rekomendujemy analizę feedbacku z tej trasy w celu identyfikacji przyczyn niższej satysfakcji.</li>
</ul>
"""
display(Markdown(wniosek_markdown))
```
...
<ul>
    <li><b style='color: #198754;'>Ulubiony typ wyprawy:</b> Klienci najwyżej oceniają format <b>{best_type['typ_wyprawy']}</b>, przyznając mu średnią ocenę <b>{best_type['srednia_ocena']:.2f}</b> (na podstawie {int(best_type['liczba_ocen'])} opinii). Warto inwestować w promocję tego segmentu.</li>
    <li style="margin-top: 15px;"><b style='color: #dc3545;'>Do przemyślenia:</b> Największe wyzwanie stanowi format <b>{worst_type['typ_wyprawy']}</b> (średnia <b>{worst_type['srednia_ocena']:.2f}</b> z {int(worst_type['liczba_ocen'])} opinii). Należy zastanowić się, czy oczekiwania klientów wobec tego typu wypraw są w pełni zaspokajane.</li>
</ul>
"""
...
def assign_segment(row):
    if row['R_score'] >= 4 and row['F_score'] >= 4: 
        return 'Mistrzowie (Champions)'
    if row['R_score'] >= 3 and row['F_score'] >= 3: 
        return 'Lojalni Klienci'
    if row['R_score'] <= 2 and row['F_score'] >= 3: 
        return 'Ryzykowni (At Risk)'
    if row['R_score'] >= 3 and row['M_score'] >= 4: 
        return 'Wieloryby (Big Spenders)'
...
fig.update_layout(showlegend=False,plot_bgcolor='white', xaxis_tickangle=-45)
fig.show()
```
```{python}
...
recommendations = {
    'Mistrzowie (Champions)': 
        "To nasi najcenniejsi klienci. Angażujmy ich, prośmy o opinie i recenzje, "
        "zaoferujmy programy lojalnościowe i dostęp do ekskluzywnych ofert. "
        "Są ambasadorami naszej marki.",
        
    'Lojalni Klienci': 
        "Są kręgosłupem naszej firmy. Podtrzymujmy ich zaangażowanie, oferując "
        "nowe produkty, lepsze wersje wypraw "
        "i budując z nimi długotrwałą relację.",

    'Ryzykowni (At Risk)': 
        "KLUCZOWA GRUPA DO REAKTYWACJI! To byli dobrzy klienci, którzy dawno "
        "nic nie kupili. Skontaktujmy się z nimi osobiście, zaoferujmy spersonalizowaną "
        "zniżkę 'na powrót' i dowiedzmy się, dlaczego przestali podróżować.",

    'Wieloryby (Big Spenders)':
        "Wydają dużo, ale niekoniecznie często. Upewnijmy się, że są zadowoleni "
        "z jakości usług. Mogą być podatni na oferty konkurencji. Warto zadbać "
        "o ich satysfakcję.",

    'Nowi Klienci':
        "Pierwsze wrażenie jest kluczowe. Zapewnijmy im doskonałe doświadczenie "
        "na pokładzie i stwórzmy spersonalizowaną ścieżkę komunikacji (np. seria maili), "
        "aby zachęcić ich do drugiej podróży i zamienić w lojalnych klientów.",
        
    'Hibernujący':
        "Duża grupa nieaktywnych klientów. Jeśli ich liczba jest znacząca, rozważmy "
        "szeroko zakrojoną, ale niskokosztową kampanię 'Tęsknimy za Tobą!' z atrakcyjną "
        "ofertą. Nie inwestujmy w nich tyle, co w segment 'Ryzykowni'."
}
...
## Wyprawy i incydenty.
W tym podpunkcie będziemy chcieli zbadać dwie rzeczy, które mogą być ze sobą powiązane. Najpierw na wykresie zbadamy to, jakie miesiące zawierają najwięcej lub najmniej wypraw, to samo zbadamy dla incydentów. Następnie postaramy się stwierdzić, czy istnieje pomiędzy tymi danymi korelacja.
```{python}
...
months_map = {
    '01': "styczniu", '02': "lutym", '03': "marcu", '04': "kwietniu",
    '05': "maju", '06': "czerwcu", '07': "lipcu", '08': "sierpniu",
    '09': "wrześniu", '10': "październiku", '11': "listopadzie", '12': "grudniu"
}

# Zamiana indexów (ints) na str z zerem wiodącym
max_trips = f"{trip_counts.idxmax():02d}"
min_trips = f"{trip_counts.idxmin():02d}"
max_incidents = f"{incidents_counts.idxmax():02d}"
min_incidents = f"{incidents_counts.idxmin():02d}"

markdown_podsumowanie = f"""
### Podsumowanie miesięczne

- Najwięcej **wycieczek** odbyło się w  **{months_map[max_trips]}**, a najmniej w  **{months_map[min_trips]}**.
- Najwięcej **incydentów** miało miejsce w **{months_map[max_incidents]}**, a najmniej w **{months_map[min_incidents]}**.
"""

display(Markdown(markdown_podsumowanie))

from scipy.stats import spearmanr  # używamy korelacji rang Spearmana


# Korelacja między liczbą wycieczek a incydentów
correlation, p_value = spearmanr(trip_counts.values, incidents_counts.values)
correlation = round(correlation, 2)

# Progi interpretacji
if correlation > 0.6:
    interpretacja = "Istnieje **silna dodatnia korelacja** – miesiące z większą liczbą lotów mają zwykle więcej incydentów."
elif correlation > 0.3:
    interpretacja = "Występuje **umiarkowana zależność** – wzrost liczby wycieczek często towarzyszy wzrostowi incydentów."
elif correlation > 0.1:
    interpretacja = "Korelacja jest słaba – nie ma jednoznacznej zależności."
else:
    interpretacja = "**Brak istotnej zależności** – liczba incydentów nie rośnie razem z liczbą wypraw."

markdown_analiza = f"""
### Czy istnieje zależność między liczbą wypraw a incydentami?

Sprawdzimy to za pomocą współczynnika korelacji Spearmana który wynosi: **{correlation}**

{interpretacja}
"""

display(Markdown(markdown_analiza))
...
Przeanalizujemy to w następujący sposób, wyznaczymy 7 kategorii:
...
gdzie "po czasie" jest najmniej chcianą opcją, zaś "91+ dni" najbardziej chcianą. Do każdej kategorii przypiszemy wagi o -3 do 3, i obliczymy średnią ważoną naszego podsumowania. Dzięki temu będziemy mogli ocenić całokształt terminów płatności.
...
# Przypisz punktację
df_summary['Punkty'] = df_summary['Okno czasowe'].map(punkty)

df_summary['Liczba płatności'] = df_summary['Liczba płatności'].astype(int)
df_summary['Punkty'] = df_summary['Punkty'].astype(int)

# Oblicz średnią ważoną
suma_wag = df_summary['Liczba płatności'].sum()
srednia_wazona = (df_summary['Punkty'] * df_summary['Liczba płatności']).sum() / suma_wag
srednia_wazona = round(srednia_wazona, 2)
...
def interpretuj_srednia(s):
    if s <= -2:
        return "Sytuacja kryzysowa – klienci płacą zdecydowanie zbyt późno."
    elif s <= -1:
        return "Zła sytuacja – przeważają płatności późne lub po czasie."
    elif s < 1:
        return "Umiarkowana sytuacja – potrzebna poprawa."
    elif s < 2:
        return "Dobra sytuacja – większość klientów płaci z wyprzedzeniem."
    else:
        return "Bardzo dobra sytuacja – klienci płacą wcześnie i planowo."

komentarz = interpretuj_srednia(srednia_wazona)

markdown_podsumowanie = f"""
### Ogólna ocena zachowania klientów

Średnia ważona punktowa dla sposobu płatności wynosi **{srednia_wazona}**.
...
fig = make_subplots(
    ...
    subplot_titles=(
        "Źródła największego przychodu według metody płatności",
        "Popularność metod płatności według liczby użyć"
    )
)
...
most_popular = df_methods.loc[df_methods['total_transactions'].idxmax(), 'payment_method']
most_cash = df_methods.loc[df_methods['total_amount'].idxmax(), 'payment_method']
...
def map(name):
    if name =="wire_transfer":
        ...
        return "karta kreadytowa"
    else:
        return name

markmarkdown_text = f"""
Z powyższych wykresów możemy wywnioskować, że najbardziej popularnym sposobem płatności jest: **{(map(most_popular))}**. Patrząc po kątem kwoty, metodą dzięki której otrzymaliśmy największą sumę pieniędzy jest: **{(map(most_cash))}**. Warto postarać się, aby płatności tymi sposobami były łatwiejsze i przyjemniejsze dla klientów oraz by nie występowały błędy.
"""
display(Markdown(markmarkdown_text))
```
