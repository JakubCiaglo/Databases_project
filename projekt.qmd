...
# 2. Zdefiniuj możliwe kategorie kosztów wraz z procentowym przedziałem w odniesieniu do base_price
cost_templates = [
...
# 3. Wygeneruj koszty: na każdą wyprawę wstaw kilka pozycji kosztowych (3-5 wpisów różnego typu)
cost_rows = []

for trip_id, base_price in trip_base_prices:
    # Rzutujemy base_price (Decimal) na float, by móc mnożyć przez float
    bp = float(base_price)

    for tpl in cost_templates:
        pct = random.uniform(tpl["min_pct"], tpl["max_pct"])
        cost_amount = round(bp * pct, 2)
        cost_rows.append((trip_id, tpl["description"], cost_amount))

# 4. Wstawianie danych do tabeli costs
cursor.executemany(
    """
    INSERT INTO costs (
