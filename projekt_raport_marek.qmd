...
# Tworzymy kolumnę z kolejną wycieczką klienta
df['next_trip'] = df.groupby('client_id')['trip_id'].shift(-1)

# Jeśli klient ma kolejną wycieczkę → oznaczamy TRUE
df['returned_after'] = df['next_trip'].notna()

# Teraz grupujemy po trip_id i liczymy ilu klientów "wróciło po tej wycieczce"
trip_retention = df[df['returned_after']].groupby('trip_id').size().reset_index(name='clients_returned')

# Sortujemy od największej retencji
