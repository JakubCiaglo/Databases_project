...
WITH Monetary AS (
    ...
    WHERE 
        ...
        transaction_date <= '{analysis_date_str}'
    GROUP BY client_id
),
RecencyFrequency AS (
    SELECT
    ...
    WHERE 
        t.departure_datetime <= '{analysis_date_str}'
...
SELECT
    ...
    c.first_name,
    c.last_name,
    DATEDIFF('{analysis_date_str}', rf.last_trip_date) AS recency_days,
...
```
```{python}
print(df_rfm["recency_days"].head(20))
print(pd.qcut(df_rfm['recency_days'], q=4, labels=False, duplicates='drop').head(20))
```
```{python}
